...
export const selectOptionListProps: PerTabSelector<OptionListViewProps> = (state, ctx) => {
	...
	return {
		...
		appendActive: state.appendMode && ctx.i === state.currentTab,
		...(confirmed ? { confirmed } : {}),
	};
};
...
export const selectPreviewPaneProps: PerTabSelector<PreviewPaneProps> = (state, ctx) => ({
	// Also true while inline append mode is active — hides the "Ctrl+E" affordance
	// under the preview while the user is already appending.
	notesVisible: state.notesVisible || state.appendMode,
	selectedIndex: state.optionIndex,
