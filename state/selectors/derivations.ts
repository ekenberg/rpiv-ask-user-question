...
export function selectConfirmedIndicator(
	...
	// Show an inline addendum (answer notes) on the confirmed row when tabbing back,
	// so text appended via Ctrl+E stays visible on the option it was attached to.
	if (prior.notes && prior.notes.length > 0) {
		return { index, labelOverride: `${prior.answer} — ${prior.notes}` };
	}
	return { index };
}
