...
/** A pre-answer note tagged with the option label it was typed on. */
...
export interface QuestionnaireState {
	...
	/**
	 * Inline append mode (Ctrl+E on a focused option of a non-preview single-select
	 * question): the option row grows an inline editor at the end of its label; typed
	 * text becomes the answer's `notes`. Enter confirms option+addendum in one step,
	 ...
	 * pass otherwise). Merged into the answer at confirm time — but ONLY when the note's
	 * `option` tag matches the option being confirmed: a note belongs to the option it was
