...
export type QuestionnaireAction =
	...
	/** Enter inline append mode on the focused option (Ctrl+E). */
	| { kind: "append_enter" }
	/** Confirm option + inline addendum in one step; `note` is the raw editor buffer. */
...
export function routeKey(data: string, state: QuestionnaireState, runtime: QuestionnaireRuntime): QuestionnaireAction {
	...
	if (state.appendMode) {
		if (kb.matches(data, KEYBIND_CONFIRM)) {
			const q = runtime.questions[state.currentTab];
			const item = runtime.currentItem;
			if (!q || item?.kind !== "option") return { kind: "append_exit" };
			return {
				kind: "append_confirm",
				answer: { questionIndex: state.currentTab, question: q.question, kind: "option", answer: item.label },
				note: runtime.inputBuffer,
				autoAdvanceTab: computeAutoAdvanceTab(state, runtime),
			};
		}
		if (kb.matches(data, KEYBIND_CANCEL)) return { kind: "append_exit" };
	...
	// Ctrl+E starts inline append on any focused single-select option (with or without
	// preview): cursor lands at the end of the option row, typed text becomes the
	// answer's `notes`. Enter confirms option+addendum in one step; Esc discards.
	// Replaces the old `n` notes editor entirely.
	if (matchesKey(data, Key.ctrl("e")) && !q.multiSelect && runtime.currentItem?.kind === "option") {
		return { kind: "append_enter" };
	}
