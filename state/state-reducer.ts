...
import type { OptionNote, QuestionnaireState } from "./state.js";

/** Label of the focused row iff it is a real option (sentinels return undefined). */
function focusedOptionLabel(state: QuestionnaireState, ctx: ApplyContext): string | undefined {
	const item = ctx.itemsByTab[state.currentTab]?.[state.optionIndex];
	return item?.kind === "option" ? item.label : undefined;
}

/**
 * The note belonging to `(tab, optionLabel)` — from the in-flight side-band first,
 * then from a committed answer for that same option. Notes are option-scoped: a note
 * typed on option A is never surfaced while option B is focused.
 */
function noteForOption(state: QuestionnaireState, tab: number, optionLabel: string): string {
	const entry = state.notesByTab.get(tab);
	if (entry && entry.option === optionLabel) return entry.note;
	const prior = state.answers.get(tab);
	if (prior?.kind === "option" && prior.answer === optionLabel && prior.notes) return prior.notes;
	return "";
}
...
function switchTabResult(state: QuestionnaireState, nextTab: number, ctx: ApplyContext): ApplyResult {
	// Option-scoped: seed the notes editor for the row the cursor lands on (option 0).
	const first = ctx.itemsByTab[nextTab]?.[0];
	const notesValue = first?.kind === "option" ? noteForOption(state, nextTab, first.label) : "";
...
const confirmHandler: Handler<"confirm"> = (state, action, ctx) => {
	...
	// Option-scoped merge: only attach the side-band note when it was typed on the very
	// option being confirmed — a note for option A must not travel onto option B.
	const pending = state.notesByTab.get(answer.questionIndex);
	if (answer.kind === "option" && pending && pending.option === answer.answer && pending.note.length > 0) {
		answer = { ...answer, notes: pending.note };
	}
...
const notesEnterHandler: Handler<"notes_enter"> = (state, _action, ctx) => {
	// Option-scoped read: only surface a note that belongs to the focused option.
	const label = focusedOptionLabel(state, ctx);
	const value = label ? noteForOption(state, state.currentTab, label) : "";
	return {
...
const notesExitHandler: Handler<"notes_exit"> = (state, _action, ctx) => {
	const trimmed = state.notesDraft.trim();
	const notes = new Map(state.notesByTab);
	const answers = new Map(state.answers);
	const label = focusedOptionLabel(state, ctx);
	const prev = answers.get(state.currentTab);
	// Mirror into the committed answer only when it is for THIS option.
	const prevMatches = prev?.kind === "option" && label !== undefined && prev.answer === label;
	if (trimmed.length === 0) {
		if (label !== undefined && notes.get(state.currentTab)?.option === label) notes.delete(state.currentTab);
		if (prevMatches && prev?.notes) {
			const stripped = { ...prev };
			delete (stripped as { notes?: string }).notes;
			answers.set(state.currentTab, stripped);
		}
	} else if (label !== undefined) {
		notes.set(state.currentTab, { option: label, note: trimmed });
		if (prevMatches && prev) answers.set(state.currentTab, { ...prev, notes: trimmed });
	}
	return {
...
const appendEnterHandler: Handler<"append_enter"> = (state, _action, ctx) => {
	// Option-scoped prefill: only a note typed on THIS option is offered for editing.
	const label = focusedOptionLabel(state, ctx);
	const value = label ? noteForOption(state, state.currentTab, label) : "";
	// The session's set_input_buffer effect also moves the cursor to end-of-buffer.
	return { state: { ...state, appendMode: true }, effects: [{ kind: "set_input_buffer", value }] };
};

const appendExitHandler: Handler<"append_exit"> = (state, _action, _ctx) => ({
	state: { ...state, appendMode: false },
...
const appendConfirmHandler: Handler<"append_confirm"> = (state, action, ctx) => {
	...
	const optionLabel = action.answer.answer ?? "";
	const notes = new Map<number, OptionNote>(state.notesByTab);
	if (trimmed.length === 0) {
		// Only clear a note that belongs to this option — another option's note survives.
		if (notes.get(action.answer.questionIndex)?.option === optionLabel) notes.delete(action.answer.questionIndex);
	} else {
		notes.set(action.answer.questionIndex, { option: optionLabel, note: trimmed });
	}
	const staged: QuestionnaireState = { ...state, notesByTab: notes, appendMode: false };
	const result = confirmHandler(
		staged,
		{ kind: "confirm", answer: action.answer, autoAdvanceTab: action.autoAdvanceTab },
		ctx,
	);
	return { state: result.state, effects: [{ kind: "clear_input_buffer" }, ...result.effects] };
};
