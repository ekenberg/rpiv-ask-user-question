...
export function buildHintText(question: QuestionData | undefined, isMulti: boolean, state: DialogState): string {
	...
	if (state.appendMode) {
		return [t("hint.append_confirm", "Enter to answer with addendum"), t("hint.append_discard", "Esc to discard")].join(
			" · ",
		);
	}
	...
	// Ctrl+E inline append — every single-select question, on a real option row.
	if (question && question.multiSelect !== true && !state.notesVisible && !state.inputMode && !state.chatFocused) {
		parts.push(t("hint.append", HINT_PART_APPEND));
	}
