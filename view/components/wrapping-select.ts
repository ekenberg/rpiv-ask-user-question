...
export class WrappingSelect implements Component {
	...
	/**
	 * Inline append mode (Ctrl+E): the ACTIVE option row renders as `label — <input>`
	 * with the live cursor, sharing the same inputBuffer/cursor plumbing as the
	 * "other" row (the two modes are mutually exclusive by construction).
	 */
	...
	setAppendMode(active: boolean): void {
	...
	private renderItem(
		...
		// Inline append mode: the active option row renders its label followed by the
		// live input (cursor at typing position). Description stays below, unchanged.
		if (item.kind === "option" && isActive && this.appendActive) {
			return [
				...this.renderInlineInputRow(rowPrefix, continuationPrefix, contentWidth, `${item.label} — `),
	...
	private renderInlineInputRow(
		...
		const raw = `${labelPrefix}${before}${CURSOR_MARKER}\x1b[7m${atCursor}\x1b[27m${after}`;
